...
> **이 문서에는 규칙만 적는다.** 진행 상황 · 화면별 결정 · 카피는
> `project-docs/` 의 해당 문서에 적는다.
> 규칙을 추가·수정할 때는 같은 주제가 이미 다른 절에 있는지 먼저 확인한다.
> 한 주제는 한 곳에만 적는다.
...
- `카피후보_팀공유_AI빌더그룹_랜딩페이지.md` — 타 팀원 카피 후보 모음 (판단 기준은 「충돌 시 우선순위」 참조)
...
- `/project-docs/PRD_AI빌더그룹_랜딩페이지.md` — 내 요구사항 명세 (팀 확정본과 별개)
...
| 영역 | 기준 |
|---|---|
| 화면 구조 · 기능 · 데이터 · 정책 | 팀 확정 문서 (`확정문서(260814)/`) |
| 색 · 폰트 · 모서리 | 시안2 토큰 (변경 불가 — 클라이언트 채택) |
| **카피 · 문구** | **`기획안_*.md` · `PRD_*.md`** |
| 여백 · 배경 리듬 · 섹션 라벨 · 인터랙션 배치 | 디자인규칙 문서 |

**카피 기준 (이 절에만 존재한다)**

- 모든 문구는 `기획안_AI빌더그룹_랜딩페이지.md` · `PRD_AI빌더그룹_랜딩페이지.md` 기준.
- `카피후보_팀공유_*.md` 는 참고 자료다. 그대로 인용하지 않는다.
  기획-2안은 팀장 작업본이며 확정 사항이 아니다 (결정시트 I-7).
- 기획안·PRD 에 해당 문구가 없으면 임의로 작성하지 말고 사용자에게 먼저 묻는다.
- 단, 아래는 예외로 변경하지 않는다: 확정 소구점 3건, POL-01 · POL-09 · POL-11①

번호 체계: 팀 확정 문서는 `REQ-` / `FN-` / `POL-`, 내 PRD는 `FR-`.
충돌하면 팀 공통 문서를 따르고, `FR-` 번호는 판단 근거용으로만 쓴다.

구조·기능에서 확정 문서와 다르게 구현해야 하면 임의로 바꾸지 말고 먼저 알린다.
