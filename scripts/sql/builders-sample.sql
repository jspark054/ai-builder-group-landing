...
--   image_url = 등록됨      — 생성 인물 사진이다 (08-24). 실존 인물이 아니므로
--                             초상권 대상이 없다. 정책 「이미지 생성」이 생성 이미지를
--                             허용하고, POL-09 의 생성 금지 항목은 「후기 · 고객 발언」에
--                             한정된다. **POL-02 이니셜 폴백 경로는 이제 타지 않는다.**
--   image_alt = 필수        — FN-A02-06. 표기명이 항상 이미지 옆에 있으므로
